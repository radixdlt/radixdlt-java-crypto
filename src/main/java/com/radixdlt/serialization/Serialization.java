...
public class Serialization {
	...
	Serialization(SerializerIds idLookup, SerializationPolicy policy) {
		...
		JacksonCborMapper hashDsonMapper = JacksonCborMapper.create(idLookup, filterProviderFor(policy.getIncludedFields(Output.HASH)),
				true, Optional.empty());
		...
		dsonBuilder.put(Output.NONE, JacksonCborMapper.create(idLookup, noneProvider, true, Optional.empty()));
		dsonBuilder.put(Output.ALL, JacksonCborMapper.create(idLookup, allProvider, true, Optional.empty()));
		for (Output e : Lists.newArrayList(Output.PERSIST, Output.WIRE)) {
			dsonBuilder.put(e, JacksonCborMapper.create(idLookup, filterProviderFor(policy.getIncludedFields(e)), true, Optional.empty()));
		}
		...
		JacksonJsonMapper hashJsonMapper = JacksonJsonMapper.create(idLookup, filterProviderFor(policy.getIncludedFields(Output.HASH)),
		...
		jsonBuilder.put(Output.NONE, JacksonJsonMapper.create(idLookup, noneProvider, false, Optional.empty()));
		jsonBuilder.put(Output.ALL, JacksonJsonMapper.create(idLookup, allProvider, false, Optional.empty()));
		for (Output e : Lists.newArrayList(Output.PERSIST, Output.WIRE)) {
			jsonBuilder.put(e, JacksonJsonMapper.create(idLookup, filterProviderFor(policy.getIncludedFields(e)), false, Optional.empty()));
		}
